...
//     // 1. Try extracting from PDF first
//     const rawPDFAnswer = await answerFromPDF(question, pdfId);

//     if (rawPDFAnswer) {
//       // 2. Format the answer for better readability
...
//     // 3. Fallback to Gemini if no PDF match
...
_.post("/ask", async (req, res) => {
  try {
    ...
    const geminiResponse = await askGemini({
      question,
      context: pdfAnswer?.context || "No PDF context",
